...
#' @description make_correspondences() is a Make function that creates a new R object. Specifically, this function implements an algorithm to make correspondences. The function is called for its side effects and does not return a value.
...
#' @keywords internal
make_correspondences <- function (data_tb = NULL, key_1L_chr = character(0), min_1L_int = 3L, 
...
#' @param erp_raw_tb Estimatedesident Population raw (a tibble)
#' @param age_range_int Age range (an integer vector)
...
#' @keywords internal
make_erp_ds <- function (erp_raw_tb, age_range_int, age_bands_lup = NULL, age_tfmn_fn = as.integer, 
    as_dyad_1L_lgl = TRUE, frequency_1L_chr = "quarterly", measure_1L_chr = "count", 
    select_chr = c("time_period", "age", "sex", "obs_value"), 
    sex_chr = c("female", "male", "person"), summarise_1L_lgl = FALSE, 
    var_ctg_chr = c("Temporal", "Key", "Key", "Metric")) 
{
    ...
    if (as_dyad_1L_lgl) {
        erp_xx <- ready4use::Ready4useDyad(ds_tb = erp_tb) %>% 
            add_dictionary(var_ctg_chr = var_ctg_chr)
    }
    else {
        erp_xx <- erp_tb
    }
...
#' @param mbs_raw_tb Medicare Benefits Schedule raw (a tibble)
#' @param as_dyad_1L_lgl As dyad (a logical vector of length one), Default: TRUE
#' @param erp_tb Estimatedesident Population (a tibble), Default: NULL
#' @param age_bands_lup Age bands (a lookup table), Default: NULL
...
#' @export 
#' @importFrom dplyr mutate case_when select filter inner_join across
#' @importFrom rlang sym
#' @importFrom ready4use Ready4useDyad
#' @keywords internal
make_medicare_ds <- function (mbs_raw_tb, as_dyad_1L_lgl = TRUE, erp_tb = NULL, age_bands_lup = NULL, 
    age_group_var_1L_chr = "AgeGroup", index_1L_chr = "Quarter", 
    key_vars_chr = c("Sex", "Age"), metrics_chr = c("Patients", 
        "Services"), new_metrics_chr = c("ServicesPerPatient", 
...
#' @param censor_1L_lgl Censor (a logical vector of length one), Default: TRUE
...
#' @param offset_1L_int Offset (an integer vector of length one), Default: -1
...
#' @export 
#' @importFrom dplyr select mutate pull filter bind_rows arrange group_by summarise across first case_when
#' @importFrom tsibble fill_gaps as_tibble as_tsibble
...
#' @importFrom tibble tibble
#' @keywords internal
make_retainers <- function (retainers_tb, as_tsibble_1L_lgl = FALSE, censor_1L_lgl = TRUE, 
    ...
    fiscal_start_1L_int = 7L, offset_1L_int = -1, provider_id_1L_chr = "ProviderID", 
    reset_new_1L_lgl = TRUE, unit_1L_chr = "days") 
{
    retainers_tsb <- update_retainers_ds(retainers_tb, cost_var_1L_chr = cost_var_1L_chr, 
        ...
        transform_to_tsibble(index_1L_chr = "Date", metrics_chr = c("Clinicians", 
            "Retainer")) %>% dplyr::select(Date, Clinicians, 
        Retainer)
    ...
    if (fill_gaps_1L_lgl) {
        retainers_tsb <- retainers_tsb %>% tsibble::fill_gaps(Clinicians = 0, 
            Retainer = 0)
        if (cumulatives_1L_lgl) {
            retainers_tsb <- retainers_tsb %>% tidyr::fill(CumulativeRetainer, 
                CumulativeClinicians)
        }
    }
    retainers_tsb <- add_temporal_vars(retainers_tsb, date_var_1L_chr = "Date", 
    ...
    if (!is.null(data_tb)) {
        ...
        if (!identical(preexisting_chr, character(0))) {
            ...
            preexisting_tsb <- preexisting_tb %>% make_retainers(as_tsibble_1L_lgl = T, 
                cumulatives_1L_lgl = cumulatives_1L_lgl, cost_var_1L_chr = cost_var_1L_chr, 
            ...
            retainers_tsb <- retainers_tsb %>% tsibble::as_tibble() %>% 
                ...
                dplyr::arrange(Date) %>% dplyr::group_by(Date) %>% 
                dplyr::summarise(dplyr::across(setdiff(names(retainers_tsb), 
                  c("Date", make_temporal_vars())), sum), dplyr::across(intersect(names(retainers_tsb), 
                  make_temporal_vars()), dplyr::first)) %>% tsibble::as_tsibble(index = Date)
            if (censor_1L_lgl) 
                retainers_tsb <- retainers_tsb %>% dplyr::filter(Date >= 
    ...
    if (dyad_1L_lgl) {
        data_xx <- Ready4useDyad(ds_tb = data_xx) %>% add_dictionary(var_ctg_chr = "Temporal")
...
#' @keywords internal
make_summary_ds <- function (data_xx, join_before_dtm = NULL, max_tenure_1L_dbl = numeric(0), 
    prefix_1L_chr = "Cumulative", separation_after_dbl = numeric(0)) 
{
    if (inherits(data_xx, "Ready4useDyad")) {
        X_Ready4useDyad <- data_xx
    }
    ...
    if (!identical(max_tenure_1L_dbl, numeric(0))) {
        X_Ready4useDyad <- renewSlot(X_Ready4useDyad, "ds_tb", 
            X_Ready4useDyad@ds_tb %>% dplyr::filter(!is.na(UID)) %>% 
                dplyr::filter(Tenure <= max_tenure_1L_dbl) %>% 
                add_cumulatives(metrics_chr = make_metric_vars(), 
                  arrange_by_1L_chr = "Date", prefix_1L_chr = "Cumulative", 
                  group_by_1L_chr = "UID") %>% dplyr::group_by(UID) %>% 
                dplyr::summarise(dplyr::across(c(Date, Referrer, 
                  Role, Sex, Age, Categorisation, Para, Aesthetic, 
                  Individual, Winter, Severity), dplyr::first), 
                  dplyr::across(c("Tenure", make_cumulatives(separation_after_dbl = separation_after_dbl)), 
                    max), dplyr::across(make_metric_vars("eoc", 
                    separation_after_dbl = separation_after_dbl)[startsWith(make_metric_vars("eoc", 
                    separation_after_dbl = separation_after_dbl), 
                    "Active")], sum)) %>% dplyr::mutate(dplyr::across(make_metric_vars("eoc", 
                separation_after_dbl = separation_after_dbl)[startsWith(make_metric_vars("eoc", 
                separation_after_dbl = separation_after_dbl), 
                "Active")], as.logical)) %>% dplyr::ungroup() %>% 
                dplyr::filter(Date <= join_before_dtm))
    }
    else {
        X_Ready4useDyad <- renewSlot(X_Ready4useDyad, "ds_tb", 
            X_Ready4useDyad@ds_tb %>% dplyr::filter(!is.na(UID)) %>% 
                dplyr::group_by(UID) %>% dplyr::summarise(dplyr::across(c(Date, 
                Referrer, Role, Sex, Age, Categorisation, Para, 
                Aesthetic, Individual, Winter, Severity, ProviderID, 
                ProviderState), dplyr::first), dplyr::across(c(Tenure, 
                !!!rlang::syms(cumulatives_chr)), max), dplyr::across(c(!!!rlang::syms(active_vars_chr)), 
                sum)) %>% dplyr::mutate(dplyr::across(c(!!!rlang::syms(active_vars_chr)), 
...
#' @description make_ts_models() is a Make function that creates a new R object. Specifically, this function implements an algorithm to make time series models. The function returns Time series models (a list).
#' @param data_xx Data (an output object of multiple potential types)
...
#' @importFrom purrr map keep_at
#' @importFrom fable MEAN NAIVE SNAIVE TSLM ETS ARIMA NNETAR
#' @importFrom fabletools model
#' @importFrom stats setNames
make_ts_models <- function (data_xx, approximation_xx = NULL, fill_gaps_1L_lgl = FALSE, 
    frequency_1L_chr = c("daily", "weekly", "monthly", "quarterly", 
        "yearly"), index_1L_chr = character(0), key_vars_chr = character(0), 
    key_totals_ls = NULL, metrics_chr = make_metric_vars(), models_chr = c("Mean", 
        "Naïve", "Seasonal naïve", "Drift", "Trend", "LMTS", 
        "ETS", "ARIMA", "NNTEAR", "Prophet", "Reg_ARIMA", "Reg_TSLM"), 
    model_type_1L_chr = "multiplicative", order_1L_int = 2, period_1L_int = 12, 
    stepwise_1L_lgl = TRUE, terms_1L_chr = character(0), test_1L_int = integer(0), 
