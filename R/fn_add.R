...
#' Add period
#' @description add_period() is an Add function that updates an object by adding new values to new or empty fields. Specifically, this function implements an algorithm to add period. The function returns Data (an output object of multiple potential types).
#' @param data_xx Data (an output object of multiple potential types)
...
#' @keywords internal
add_period <- function (data_xx, period_ctg_1L_chr = "Temporal", period_var_1L_chr = "Period", 
    tenure_var_1L_chr = "Tenure") 
{
    ...
    X_Ready4useDyad <- renewSlot(X_Ready4useDyad, "ds_tb", X_Ready4useDyad@ds_tb %>% 
        dplyr::mutate(`:=`(!!rlang::sym(period_var_1L_chr), purrr::map_int(!!rlang::sym(tenure_var_1L_chr), 
            ~max(ceiling(.), 1)))))
    X_Ready4useDyad <- X_Ready4useDyad %>% ready4use::add_dictionary(new_cases_r3 = ready4use_dictionary() %>% 
        ready4use::renew.ready4use_dictionary(var_nm_chr = period_var_1L_chr, 
            var_ctg_chr = period_ctg_1L_chr, var_desc_chr = period_var_1L_chr, 
...
{
    x_ready4show_correspondences <- make_new_correspondences(data_tsb, 
