calculate_autocorrelations <- function(data_xx,
  ...
  autocorrelations_ls <- metrics_chr %>% purrr::map(~ get_tsibble(data_xx, frequency_1L_chr = frequency_1L_chr,
                                                                  key_totals_ls = key_totals_ls, key_vars_chr = key_vars_chr,
                                                                  metrics_chr = metrics_chr) %>% tsibble::fill_gaps() %>%
                                                      feasts::ACF(!!rlang::sym(.x),
